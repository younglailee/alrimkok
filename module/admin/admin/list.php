...
<div id="<?= $module ?>">
    <div class="search">
        ...
        <tr>
            ...
            <th><label for="sch_mb_level">등급</label></th>
            <td>
                <select name="sch_mb_level" class="select" title="검색컬럼">
                <option value="">전체</option>
        ...
        if (is_array($list)) {
            for ($i = 0; $i < count($list); $i++) { ?>
                <tr class="list_tr_<?= $list[$i]['odd'] ?>">
                    ...
                    <td><?= $list[$i]['no'] ?></td>
                    <td>
                        <a href="./write.html?<?= $pk ?>=<?= $list[$i][$pk] ?>&page=<?= $page ?><?= $query_string ?>"><?= $list[$i]['mb_id'] ?></a>
                    </td>
                    <td>
                        <a href="./write.html?<?= $pk ?>=<?= $list[$i][$pk] ?>&page=<?= $page ?><?= $query_string ?>"><?= $list[$i]['mb_name'] ?></a>
                    </td>
