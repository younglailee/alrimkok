...
function submitWriteForm(f) {
    // editor
    //oEditors.getById["mb_memo"].exec("UPDATE_CONTENTS_FIELD", []);
    ...
    if (f.mode.value == "insert") {
        // 아이디 검사
        if (!validateMemberId(1)) {
            return false;
        }
    }
    // 패스워드 검사
    if (f.mb_pw.value && f.mb_pw.value != f.mb_pw_confirm.value) {
        alert("비밀번호가 일치하지 않습니다.");
        f.mb_pw_confirm.focus();
        return false;
    }
