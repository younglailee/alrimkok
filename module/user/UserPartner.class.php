...
class UserPartner extends User
{
    ...
    protected function initUpdate()
    {
        ...
        $update_columns .= ',mb_hp,mb_memo,flag_book,mb_depart,flag_live';
    ...
    protected function makeDbWhere()
    {
        ...
        if ($sch_mb_level) {
            $db_where .= " AND mb_level='$sch_mb_level'";
        }
        if ($sch_flag_use) {
            $db_where .= " AND flag_use='$sch_flag_use'";
        }
        if ($sch_flag_notice == 'Y') {
            $db_where .= " AND flag_notice='Y'";
        } elseif ($sch_flag_notice == 'N') {
            $db_where .= " AND (flag_notice='' OR flag_notice IS NULL)";
        }
        if ($sch_company) {
    ...
    public function selectDetailUser($cp_id)
    {
