...
/* init Class */
$oCrawl = new CrawlAdmin();
$oCrawl->init();
$pk = $oCrawl->get('pk');

/* check auth */
if (!$oCrawl->checkListAuth()) {
    Html::alert('권한이 없습니다.');
}

/* list */
$list = $oCrawl->selectList();
$cnt_total = $oCrawl->get('cnt_total');

/* search condition */
$search_like_arr = $oCrawl->get('search_like_arr');
$search_date_arr = $oCrawl->get('search_date_arr');
$query_string = $oCrawl->get('query_string');

/* pagination */
$page = $oCrawl->get('page');
$page_arr = $oCrawl->getPageArray();
                ...
                <table class="list_table border odd">
                    ...
                    for ($i = 0; $i < count($list); $i++) {
                        ...
                        <tr class="list_tr_<?= $list[$i]['odd'] ?>">
                            ...
                            <td>
                                <a href="./write.html?bz_id=<?= $list[$i]['bd_id'] ?>" target="_blank"
