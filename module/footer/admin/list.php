        ...
        if (is_array($list)) {
            for ($i = 0; $i < count($list); $i++) {
                ...
                <tr class="list_tr_<?= $list[$i]['odd'] ?>">
                    ...
                    <td>
                        <button type="button" onclick="changeOrder('up', this)" class="btn_change_order" title="위로"><i
                                class="xi-caret-up"></i></button>
                        <button type="button" onclick="changeOrder('down', this)" class="btn_change_order" title="아래로">
                            <i class="xi-caret-down"></i></button>
                    </td>
                    <td>
                        <a href="./write.html?<?= $pk ?>=<?= $list[$i][$pk] ?>&page=<?= $page ?><?= $query_string ?>"><img
                                src="<?= $list[$i]['thumb_uri'] ?>" width="100" alt="<?= $list[$i]['img_alt'] ?>"/></a>
                    </td>
                    <td class=""><a
                            href="./write.html?<?= $pk ?>=<?= $list[$i][$pk] ?>&page=<?= $page ?><?= $query_string ?>"><?= $list[$i]['ft_subject'] ?></a>
                    </td>
                    <td class="state"><strong
                            class="<?= $list[$i]['state_class'] ?>"><?= $ft_is_display_arr[$list[$i]['ft_is_display']] ?></strong>
                    </td>
                    <td class="button"><a
