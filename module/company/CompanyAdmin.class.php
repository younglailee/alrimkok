...
class CompanyAdmin extends Company
{
    ...
    protected function makeDbWhere()
    {
        ...
        if ($sch_partner) {
            ...
            } elseif ($sch_partner == 'head_office') {
    ...
    protected function postUpdate($arr)
    {
        ...
        if($arr['flag_book'] == 'Y'){
            ...
            $cp_id = $arr['cp_id'];
            Db::update('tbl_book_batch',"partner_id='$partner_id'","WHERE bt_company = '$cp_id'");
            Db::update('tbl_user',"flag_book='Y'","WHERE mb_id='$partner_id'");
        }
    ...
    public function selectPartner()
    {
        ...
        $select_columns = 'DISTINCT partner_id, partner_name';
        $db_where = "WHERE 1=1 AND NOT (partner_id = '' OR partner_id = 'adpartner')";
        $db_having = $this->get('db_having');
        $db_order = 'ORDER BY partner_name ASC';
        $list = Db::select($select_table, $select_columns, $db_where, $db_having . ' ' . $db_order, '');

        $result = [];
        if (is_array($list)) {
        for ($i = 0; $i < count($list); $i++) {
            $result[$list[$i]['partner_id']] = $list[$i]['partner_name'];
        }
        }
        return $result;
    }
