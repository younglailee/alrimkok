function createSEditor2(elIRField, htParams, elSeAppContainer){
	...
	if(htParams.bSkipXssFilter !== true){
		...
		oEditor.registerPlugin({
			...
			_rxFilter:/<\/*(?:applet|b(?:ase|gsound|link)|embed|frame(?:set)?|ilayer|l(?:ayer|ink)|meta|object|s(?:cript|tyle)|title|xml)[^>]*?>/gi,
	...
	// Alpha-Edu
	if (htParams.nMinWidth) {
		htDimension.nMinWidth = htParams.nMinWidth;
	}
    ...
    // 줄 바꿈 태그 변경 BR 적용
	oEditor.registerPlugin(new nhn.husky.SE_WYSIWYGEnterKey("BR"));							// 엔터 시 처리, BR로 처리
